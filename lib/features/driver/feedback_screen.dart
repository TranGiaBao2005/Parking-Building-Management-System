          ...
          final formPanel = SingleChildScrollView(
            ...
            child: Column(
              ...
              children: [
                ...
                const Text(
                        'Báo cáo vấn đề như mất thẻ, phí sai hoặc slot bị chiếm.',
                    ...
                    child: Row(
                      children: [
                        const Icon(Icons.check_circle,
                            color: AppColors.available, size: 28),
            ...
            child: Column(
              ...
              children: [
                ...
                if (myFeedbacks.isEmpty)
                  ...
                  ...myFeedbacks.map((fb) => Padding(
                        padding: const EdgeInsets.only(bottom: 10),
                        child: _FeedbackHistoryCard(feedback: fb, dtFmt: dtFmt),
                      ))
                else
                  Expanded(
                    child: ListView.separated(
                      ...
                      itemBuilder: (_, i) => _FeedbackHistoryCard(
                        feedback: myFeedbacks[i],
                        dtFmt: dtFmt,
                      ),
                    ),
                  ),
...
class _FeedbackHistoryCard extends StatelessWidget {
  final FeedbackReport feedback;
  final DateFormat dtFmt;

  const _FeedbackHistoryCard({required this.feedback, required this.dtFmt});

  @override
  Widget build(BuildContext context) {
    final isResolved = feedback.status == FeedbackStatus.resolved;
    final statusColor = isResolved ? AppColors.available : AppColors.reserved;
    return Container(
      width: double.infinity,
      ...
      child: Column(
        ...
        children: [
          ...
          const SizedBox(height: 5),
          Text(dtFmt.format(feedback.createdAt),
              style: const TextStyle(color: AppColors.textMuted, fontSize: 10)),
          if (isResolved && feedback.resolution != null) ...[
            const SizedBox(height: 8),
            Container(
              width: double.infinity,
              ...
              child: Text(
                '✅ ${feedback.resolution}',
                style:
                    const TextStyle(color: AppColors.available, fontSize: 11),
              ),
            ),
          ],
        ],
